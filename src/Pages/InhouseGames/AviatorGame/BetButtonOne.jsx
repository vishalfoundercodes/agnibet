...
function BetButtonOne({ setBtn, setBetApiHitted,  }) {
  ...
  const cancelNormalBetHandler = async () => {
    ...
    try {
      const res = await axios.get(
        `${configModalWinBhai}aviator_bet_cancel?userid=${userId}&number=1&gamesno=${sr}`
      );
      // toast.success(res?.data?.message)
  ...
  return (
    <div className="w-full py-1  lg:py-0  text-sm xsm:text-sm px-2 lg2:px-6  bg-blackAviator2 rounded-md flex flex-col items-center">
      ...
      <div className="grid grid-cols-5 gap-2 w-full mb-2">
        <div className="col-span-2 flex flex-col items-center w-full ">
          ...
          {isAuto && (
            <div className="w-full -mt-1 text-sm xsm:text-sm text-gray flex items-center justify-center">
              Auto Bet &nbsp;
              <button>
                {isAutoBetAndCashout.autoBet ? (
                  <BiToggleRight
        ...
        {betStatus ? (
          ...
          <div className="mt-2 col-span-3 w-full">
            ...
            {isAuto && (
              <div className="text-ssm xsm:text-sm flex items-center justify-center gap-1 xsm:gap-2">
